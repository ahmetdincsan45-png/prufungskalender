...
- Data model: tables `exams`, `visits`, `subjects`, `admin_credentials` created on first run; see initializers in [app.py](app.py#L38-L112).
...
- Local dev:
  ...
  - Run with `python app.py` (dev server prints DB path). Health check at [/health](app.py#L1180-L1201).
...
## Routes & Behaviors
- [/](app.py#L121-L142): Renders next upcoming exam; “after 18:00” cutoff uses Europe/Berlin if `zoneinfo` available.
- [/events](app.py#L144-L418): Returns exam events and weekday-only background ranges for holidays.
...
- [/add](app.py#L420-L478): Accepts `subjects` (comma-separated) and `date` (`YYYY-MM-DD`). Past dates are redirected to index. Inserts one `exams` row per subject.
- [/delete](app.py#L480-L558): Lists future exams plus last 10 past; allows deleting future exams only from this page.
- Admin area:
  - [/stats](app.py#L645-L1160): Username+password auth stored in `admin_credentials`; cookie `stats_auth` is SHA-256 over the password hash. Manage `subjects`, view visits, send report, logout.
  - [/stats/delete-past](app.py#L560-L643): Delete past exams (requires stats auth cookie).
  - [/stats/json](app.py#L1370-L1409): JSON stats (requires stats auth cookie).
  - [/send-report](app.py#L1531-L1585): Sends weekly email report (uses Gmail SMTP settings in [app.py](app.py)).
  - [/admin/reset](app.py#L1411-L1451), [/admin/info](app.py#L1453-L1475): Protected by env tokens (`ADMIN_RESET_TOKEN`, `ADMIN_INFO_TOKEN`).
...
- Grade defaults to `4A`; subject lists merge defaults and DB (see `/api/subjects` in [app.py](app.py#L1203-L1246)).
...
- Visit logging: every non-static request stores `ip`, `user_agent`, `path` into `visits` unless bot-like; your changes should preserve this lightweight logging.
- Auth: Stats auth is cookie-based; use helpers `_stats_expected_token()` and update flows in [app.py](app.py#L1477-L1530) when touching credentials.
...
- Email: SMTP credentials are currently hardcoded in [app.py](app.py#L22-L33). Prefer environment variables for new integrations.
